"""
Conversational RAG — chat interface with history-aware query rewriting.
Manages chat history, rewrites follow-up questions, then delegates to run_rag().
...
def rewrite_query(llm, chat_history, new_question):
    ...
    try:
        ...
        if rewritten:
            print(f"[Chat] Rewritten query: {rewritten}")
            return rewritten
    except Exception as e:
        print(f"[Chat] Rewriting failed ({e}), using original query")
